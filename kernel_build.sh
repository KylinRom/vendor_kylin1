...
MAINTENANCE=$(cat $DIR/vendor/kylin/vendor.mk | grep 'ROM_VERSION_MAINTENANCE := *' | sed  's/ROM_VERSION_MAINTENANCE := //g')
TAG=$(cat $DIR/vendor/kylin/vendor.mk | grep 'ROM_VERSION_TAG := *' | sed  's/ROM_VERSION_TAG := //g')
...
if [ -f $DIR/out/target/product/$DEVICE/KylinRom-$DEVICE-kernel-signed.zip ]; then
   mv $DIR/out/target/product/$DEVICE/KylinRom-$DEVICE-kernel-signed.zip $DIR/out/target/product/$DEVICE/KylinRom-$DEVICE-$VERSION-kernel-$DATE-signed.zip
else
